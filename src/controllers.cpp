...
pid_controller::pid_controller() : error_sum(0), output(0), error_prev(0) {}

float pid_controller::clamp(float input, float upper_limit, float lower_limit) {
  if(input < lower_limit)
    return lower_limit;
  else if(input > upper_limit)
    return upper_limit;
...
float pid_controller::loop(float error, const pid_config& config) {
  ...
  output = clamp(config.Kp*error + config.Ki*error_sum + config.Kd*derror, config.upper_limit, config.lower_limit);
